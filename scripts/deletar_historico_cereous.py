"""
WMS Kiwkiw — Script de limpeza: apaga histórico de movimentação do seller Cereous.

Execute com:
    python scripts/deletar_historico_cereous.py

O script:
  1. Localiza o seller Cereous no banco
  2. Exibe um resumo do que será apagado
  3. Pede confirmação antes de deletar
  4. Apaga stock_movements e zera stock_positions do seller
...
cur.execute("""
    ...
    FROM sellers
    WHERE UPPER(trade_name) LIKE '%CEREO%'
       OR UPPER(name)       LIKE '%CEREO%'
""")
sellers = cur.fetchall()

if not sellers:
    print("[AVISO] Nenhum seller com nome contendo 'Cereous' encontrado.")
    con.close()
    sys.exit(0)
